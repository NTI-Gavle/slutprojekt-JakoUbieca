...
App.evolve = function () {
  ...
  var deadEl = document.getElementsByClassName('dead')[0];
  if (deadEl) deadEl.textContent = this.deathCount;

  var aliveEl = document.getElementsByClassName('alive')[0];
  if (aliveEl) aliveEl.textContent = this.particles.length;

  var fpsEl = document.getElementsByClassName('fps')[0];
  if (fpsEl) fpsEl.textContent = Math.floor(1000 / (time2 - time1));

  var drawnEl = document.getElementsByClassName('drawn')[0];
  if (drawnEl) drawnEl.textContent = this.drawnInLastFrame;
...
App.draw = function () {
  ...
  this.ctx.beginPath();
  this.ctx.rect(0, 0, this.width, this.height);
  this.ctx.fillStyle = 'rgba(26, 21, 17, 0.1)';
  this.ctx.fill();
  this.ctx.closePath();
